 ...
 * 前期(4/1–8/7): 4–5月=前半(1Q) / 6–7月=後半(2Q)
 * 後期(9/1–翌2/7): **9–11月=前半(3Q)** / 12–2月=後半(4Q)
 *
 * 🔴**2026-08-27修正: 9月が「前期の後半」になっていた。**
 * 旧コメントは「前期(4–9月)/後期(10–3月)」という月区分で、`src/timetableEvents/termBounds.ts` の
 * 学年暦（前期4/1–8/7・後期9/1–翌2/7）と**9月について逆を向いていた**。後期開始は9/11＝3Q(前半)
 * なのに `'second'`(後半) を返していた。
 *
 * ⚠**これは薄表示だけの問題ではない。** `representativeClass()` が**積みコマでどちらを「その授業」と
 * するか**をこの値で決めており、`homeBanner.ts:57` / `focusClass.ts:59,107` / `widget/viewModel.ts:98`
 * が使う＝**出席バナーが狙う授業・今日の授業・ウィジェットが揃ってずれる。**
 *
 * ⚠**今まで踏まなかったのは、後期の時間割自体が取れていなかったから**（学期セレクタが「前期」固定
 * だった）。2026-08-27にそれを直した結果、この経路が初めて到達可能になった＝**同じ版で直す。**
 *
 * ⚠学期の境界は `termBounds.ts` が正典。ここが持つのは**学期内の前半/後半の境界だけ**で、
 * それはCLASSがどこにも公開しない（[[letus-class-quarter-courses]]）＝月による近似しかない。
 * **手動トグル(`pref`)が最優先**なので、外れてもユーザーが直せる。
 */
export function defaultCurrentQuarter(now: Date): Quarter {
  const m = now.getMonth() + 1
  return m === 4 || m === 5 || m === 9 || m === 10 || m === 11 ? 'first' : 'second'
}
