import { useState } from 'react'
import { Pressable, ScrollView, StyleSheet, View } from 'react-native'
import { Text } from '../ui/Text'
import { LinearGradient } from 'expo-linear-gradient'
import { useSafeAreaInsets } from 'react-native-safe-area-context'
import { COLORS } from '../theme'
import { TERMS_VERSION } from '../legal/termsVersion'
import { saveTermsConsent } from '../storage/termsConsentStore'

// 規約本文（正典は docs/legal/terms-ja.md。片方を更新したら他方も必ず合わせること）。
const TERMS_BODY = `本アプリ「リタス（Litus）」は、東京理科大学の公式アプリではない非公式のアプリです。個人が学習・利便のために提供しています。ご利用の前に以下に同意してください。

1. 本人利用・認証情報
・あなた自身のTUSアカウントでログインして利用してください。
・本アプリは認証情報（ID・パスワード）を保存しません。ログインは大学の公式ログイン画面で行われます。
・大学のSSO（シングルサインオン）が発行する認証情報の一部は端末内に保持され、アプリを終了して再度開いた際に自動的に再ログインされることがあります。LETUS・CLASS のセッションがいつ切れるかは大学側の設定によります。アプリが終了時に消すことはしません。保持をやめたい場合は、アプリ内の「設定」からいつでも消去できます。ご自身の端末を他の方が解錠できる状態にしないようご注意ください。

2. 取得する情報の範囲
・CLASSからは履修科目・時間割・出席・掲示のみを取得します。成績など機微な情報は取得しません。
・本アプリは、あなたの操作に応じてCLASSへ書き込みます（出席の登録、リアクションペーパーの提出、掲示の詳細を開いたときの既読化と、フラグの付け外し）。あなたの操作を起点としない書き込みは行いません（出席の送信が届かなかったと確認できた場合に限り、操作の直後に1回だけ自動で再送します）。
・本アプリが自ら通信する先は、LETUS／CLASSおよび大学のログイン基盤／稼働状況の確認先（waiteu.dev：不具合時や大学からの要請時に機能を停止するための、停止指示ファイルの取得のみ）／本アプリの自前バックエンド（api.waiteu.dev）に限られます。
・本アプリは、自前バックエンド（api.waiteu.dev）に対して、利用者個人を特定できる形での情報の送信を行いません。ただし、「フィードバックを送る」からご自身で不具合報告・ご要望を送信された場合は例外です。送信されるのは、選択した種別・対象・入力されたコメント・（出席またはリアクションペーパーに関する不具合の場合のみ）該当する診断記録（該当授業の科目名や、CLASS の画面から抜き出した短い診断行を含むことがあります）・通知の状況（出席の予約件数・重複の件数・旧形式の件数・次回の出席通知の時刻・課題などの予約件数・予約の失敗件数・当日に受付開始のお知らせを実際に出したコマ数の、数字と時刻だけの1行。科目名や通知の本文は含みません）・アプリの版とOS情報・端末機種・任意で入力された返信用メールアドレスに限られ、学籍番号・氏名は含まれません。通知そのものは端末内でローカルに予約され、通知の内容が送信されることはありません。お送りいただいた報告は、開発者が気づけるように Discord の開発者用チャンネルへも通知されます。通知されるのは、報告の種類と対象の機能・コメントの先頭200文字（200文字以内の報告は全文）・アプリの版・OS とその版・返信先のメールアドレスです。
・公開前のお知らせメールの事前登録は、本アプリの機能ではなくWebページ（litus.waiteu.dev）のフォームです。ご自身で登録された場合に限り、メールアドレスが自前バックエンド（api.waiteu.dev）へ送信・保存されます（保存はメールアドレス・登録元・登録日時のみ。IPアドレスは保存しません）。
・アプリ内の学食・図書館・時刻表などのリンクは、タップすると端末のブラウザで外部サイトが開くもので、本アプリが情報を送信するものではありません。

3. 禁止事項
・CLASS／LETUS／大学のシステムに過度な負荷をかける行為。
・出席コードの自動連投・総当たり、スクレイピングの乱用など、正常な運用を妨げる行為。
・代理出席・なりすまし等の不正な出席登録。
・大学の規程・利用規約・法令に違反する利用。

4. 自己責任・免責
・本アプリの利用に伴う一切の責任は利用者が負います。大学規程・法令の遵守は利用者の責任です。
・本アプリは現状有姿で提供され、いかなる保証もありません。利用に起因する損害について、提供者は責任を負いません。

5. サービスの一時停止・終了
・重大な不具合・脆弱性が判明した場合、本アプリの全部または一部の機能を予告なく一時停止することがあります。
・大学から利用の停止・変更等の要請を受けた場合、その要請に従い、機能の一時停止または終了を行うことがあります。
・可能な範囲で周知に努めますが、停止・終了に起因する損害について提供者は責任を負いません。

6. 規約の改定
・規約は改定されることがあります。改定時は起動時に再度の同意を求めます。
・本規約は日本法に準拠します。

「同意して始める」を押すと、上記に同意したものとみなします。`

export default function TermsConsentScreen({ onAccept }: { onAccept: () => void }) {
  const insets = useSafeAreaInsets()
  const [busy, setBusy] = useState(false)
  async function accept() {
    if (busy) return
    setBusy(true)
    try {
      await saveTermsConsent(TERMS_VERSION)
      onAccept()
    } finally {
      setBusy(false)
    }
  }
  return (
    <LinearGradient colors={[COLORS.gradTop, COLORS.gradBottom]} style={styles.root}>
      <View style={[styles.inner, { paddingTop: insets.top + 24, paddingBottom: insets.bottom + 16 }]}>
        <Text style={styles.title}>ご利用の前に</Text>
        <Text style={styles.sub}>リタスは非公式アプリです。以下に同意のうえご利用ください。</Text>
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollBody}>
          <Text style={styles.body}>{TERMS_BODY}</Text>
        </ScrollView>
        <Pressable style={[styles.cta, busy && { opacity: 0.6 }]} onPress={accept} disabled={busy}>
          <Text style={styles.ctaText}>同意して始める</Text>
        </Pressable>
      </View>
    </LinearGradient>
  )
}

const styles = StyleSheet.create({
  root: { flex: 1 },
  inner: { flex: 1, paddingHorizontal: 20 },
  title: { color: COLORS.white, fontSize: 24, fontWeight: '700', marginBottom: 6 },
  sub: { color: COLORS.whiteSubtle90, fontSize: 13, marginBottom: 14 },
  // padding は ScrollView 本体ではなく contentContainerStyle 側へ。本体に付けると最後の行が枠下に
  // 隠れて見切れる（RNのScrollViewのpadding挙動）。下部は同意ボタン手前まで十分な余白を確保する。
  // 規約本文の読書面（固定緑ブランド地に乗るほぼ不透明の白パネル・テーマ非追従）。
  scroll: { flex: 1, backgroundColor: 'rgba(255,255,255,0.94)', borderRadius: 14 }, // design-allow
  scrollBody: { padding: 16, paddingBottom: 28 },
  body: { color: COLORS.ink, fontSize: 13.5, lineHeight: 22 },
  cta: { marginTop: 16, backgroundColor: COLORS.white, borderRadius: 999, paddingVertical: 15, alignItems: 'center' },
  ctaText: { color: COLORS.emeraldDark, fontSize: 16, fontWeight: '700' },
})
