...
・大学のSSO（シングルサインオン）が発行する認証情報の一部は端末内に保持され、アプリを終了して再度開いた際に自動的に再ログインされることがあります。LETUS・CLASS自体のセッションはアプリの終了時に失効します。保持をやめたい場合は、アプリ内の「設定」からいつでも消去できます。ご自身の端末を他の方が解錠できる状態にしないようご注意ください。
...
・本アプリは、自前バックエンド（api.waiteu.dev）に対して、利用者個人を特定できる形での情報の送信を行いません。ただし、「フィードバックを送る」からご自身で不具合報告・ご要望を送信された場合は例外です。送信されるのは、選択した種別・対象・入力されたコメント・（出席またはリアクションペーパーに関する不具合の場合のみ）該当する診断記録（該当授業の科目名や、CLASS の画面から抜き出した短い診断行を含むことがあります）・通知の予約状況（予約件数・失敗件数・次回予約時刻のみの1行。科目名や通知の本文は含みません）・アプリの版とOS情報・端末機種・任意で入力された返信用メールアドレスに限られ、学籍番号・氏名は含まれません。通知そのものは端末内でローカルに予約され、通知の内容が送信されることはありません。お送りいただいた報告は、開発者が気づけるように Discord の開発者用チャンネルへも通知されます。通知されるのは、報告の種類と対象の機能・コメントの冒頭・アプリの版・OS とその版・返信先のメールアドレスです。
