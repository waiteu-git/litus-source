...
export function parseAttendanceMessage(raw: string): AttendanceReception {
  ...
  // 2) 受付なし
  // 🔴**リアペを状態で塞いではいけない。この行を「塞ぐ」側へ書き換えないこと。**
  //
  // NONE_MARKER（「現在、出席確認中の履修授業はありません」）は**2つの別の世界で同じ文言が出る**:
  //   (a) 学期中・受付時間外 …… 履修授業は在る。**リアペの入力フォームは開ける**
  //       （ユーザーの前期中の実使用。授業時間外でも押した先にフォームがあった）
  //   (b) 休業期間・履修授業ゼロ … 押しても同じ画面へ戻り、**フォームは開かない**
  //       （2026-08-13 実機・手動押下で確認）
  // **表示は同一で、この2つを区別する材料がページに無い。** ⇒ 状態からリアペの可否は予測できない。
  //
  // (b) の観測だけで塞ぐと、**(a) すなわち学期中の授業時間外のリアペ提出を丸ごと殺す**。
  // 後期開始は9/11＝公開直後で、中核機能が壊れる。**誤りの向きが致命的に非対称**なので塞がない。
  //
  // ⚠**この判断は3回反転した。根拠が違うので混同しないこと:**
  //   1回目 `3ff1956` … form-missing からの**推測**で塞いだ（推測の範囲を確かめていない）
  //   2回目 `e83939f` … 「実際は提出できる」の見立てで撤回
  //   3回目 `e9be781` … 休業期間の**手動押下の観測**で塞ぎ直した（観測はしたが**世界が1つだと思い込んだ**）
  //   4回目 ここ ……… 学期中は開けたという事実が出て撤回。**観測は正しく、一般化が誤りだった**
  //
  // 予測できない以上、正しい設計は「約束しない・失敗を正直に伝える」。失敗時の文言は
  // AttendanceEngineProvider の form-missing 分岐（「提出フォームが開きませんでした」）が担う。
