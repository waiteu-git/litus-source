 ...
 *  3. 出席の開始と終了前が、たまたま同じ時刻になった時（例: 1限の終了前と、その時刻に始まる2限の開始）。
 *     🔴 2026-09-12（N1・撤回1）まで、ここは「同一曜限に積まれた別科目の出席開始」だった＝積みコマを20秒ずらして
 *     2通鳴らしていた。今は同じ種類・同じ時刻の出席を1枠にまとめてから渡す（attendanceSchedule.mergeAttendanceNotices）
 *     ので、積みコマはここに来ない。出席の鍵は枠の id（ずらしても id は変わらない）。
 *
