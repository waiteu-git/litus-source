 ...
 *
 * **実測値ではなく目安**。`mailto:` の本文長の上限はOSでもメールアプリでも規格化されておらず、
 * 端末側で黙って切られることがある。よって「切れないURLを作る」のではなく
 * **超えたら「本文をコピー」を主動線に切り替えて知らせる**という設計にしている
 * （切れたかどうかはユーザーにも我々にも見えないので、静かに落とすのが一番悪い）。
 */
