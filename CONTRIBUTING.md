...
fork など、GitHub の利用規約 D.5 に基づく行為は誰でも行えます。本プロジェクトへの貢献の準備・提出に必要な改変と、その改変の公開（fork リポジトリと、公開ミラー宛ての PR）は、[LICENSE](LICENSE) の「■ 貢献」節の**貢献目的の例外として明示的に許可されています**（改変が許されるのは貢献の準備・提出に必要な範囲だけです。GitHub の利用規約 D.5 に基づく行為（fork、GitHub 上での閲覧・表示など）と、貢献のための改変の公開（fork リポジトリと PR）を除き、目的にかかわらず再配布はできず、fork やその Releases・Pages を使ってビルド済みアプリを配布することもできません）。
